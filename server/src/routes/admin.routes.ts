...
router.get('/statistics', async (req, res) => {
  try {
    ...
    const todayUsers = await UserAdmin.countDocuments({
    ...
    const weekUsers = await UserAdmin.countDocuments({
...
// 支付结算：获取提现申请列表
router.get('/withdrawals', authenticateJWT, isAdmin, async (req, res) => {
  try {
    const page = parseInt((req.query.page as string) || '1', 10);
    const pageSize = parseInt((req.query.pageSize as string) || '10', 10);
    const skip = (page - 1) * pageSize;
...
// 获取用户列表
router.get('/user/list', authenticateJWT, isAdmin, async (req, res) => {
  try {
...
// 添加用户
router.post('/user/add', authenticateJWT, isAdmin, async (req, res) => {
  try {
    const { 
      username, 
      password, 
...
// 审核用户
router.post('/user/audit', authenticateJWT, isAdmin, async (req, res) => {
  try {
...
// 更新用户状态
router.post('/user/status', authenticateJWT, isAdmin, async (req, res) => {
  try {
...
// 分配用户角色
router.post('/user/role', authenticateJWT, isAdmin, async (req, res) => {
  try {
...
// 批量删除用户
router.post('/user/delete', authenticateJWT, isAdmin, async (req, res) => {
  try {
